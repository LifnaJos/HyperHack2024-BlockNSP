...
const verifyAadhaar = async (req,res) => {
    try {
        ...
        if(aadharNo.length == 12) {                   //Entered input is Aadhaar number.
            const validAadhaar = await db.query(`SELECT * FROM uidai_aadhar_det WHERE aadhar_no=$1`,[aadharNo]);
            console.log(validAadhaar);
            if (validAadhaar.rows.length != 0) {             //if aadhaar number is valid.
                console.log(validAadhaar);
                const existingAadhaar = await db.query(`SELECT * FROM applicant_reg WHERE "Aadhar_no"=$1`,[aadharNo]);
